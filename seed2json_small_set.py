...
shutil.rmtree("output_small", ignore_errors=True, onerror=None) 
os.mkdir("output_small")
os.mkdir("output_small/edges") 
...
# break down into phases - step one create json outputs...
# step two merge/edges
# step four hydrate neo4j

# create insurance nodes

# create lab nodes

# use patients verbatim (instead of existing patients)

# overwrite doctors with provider info

# add edges - lab output to insurance, doctors to insurance accepted, lab results to patient, doctor, insurance

# map lab output observation date to encounter date (use this date to drive encounter date... not all encounters just some)
...
# create patients json file 
print "Generating patients..." 
attributes = ATTS.PERSONAL + ATTS.PHYSICAL + ATTS.CONTACT + ATTS.SECURITY
patients_doc = Commandset("ADD", "PATIENT", content).gen_add_commandset(range(0, 100), attributes)
write_json("output_small/patients.json", patients_doc)

# create doctors json file from next 2k records
print "Generating doctors..."
attributes = ATTS.PERSONAL + ATTS.CONTACT + ATTS.SECURITY
doctors_doc = Commandset("ADD", "DOCTOR", content).gen_add_commandset(range(101, 110), attributes)
write_json("output_small/doctors.json", doctors_doc)
...
hospitals_doc = Commandset("ADD", "HOSPITAL", content).gen_add_commandset(range(111, 120), attributes)
...
write_json("output_small/hospitals.json", hospitals_doc)
...
attributes = ATTS.CONTACT
clearing_houses_doc = Commandset("ADD", "CLEARINGHOUSE", content).gen_add_commandset(range(121, 125), attributes)
...
write_json("output_small/clearinghouse_orgs.json", clearing_houses_doc)

# create research orgs json file from next 100 records (leverage contact fields only)
print "Generating research organizations..."
attributes = ATTS.CONTACT
research_orgs_doc = Commandset("ADD", "RESEARCHORG", content).gen_add_commandset(range(126, 140), attributes)
...
write_json("output_small/research_orgs.json", research_orgs_doc)

# create researchers json file from next 1k records
print "Generating researchers..."
attributes = ATTS.PERSONAL + ATTS.CONTACT + ATTS.SECURITY
researchers_doc = Commandset("ADD", "RESEARCHER", content).gen_add_commandset(range(141, 170), attributes)
write_json("output_small/researchers.json", researchers_doc)
...
# write to 
write_json("output_small/encounters.json", encounters_doc)
...
write_json("output_small/edges/encounter_edges.json", encounters_edges_doc)
...
write_json("output_small/edges/clearinghouse_edges.json", clearinghouse_edges_doc)

# create research org nodes
research_org_edges_doc = []
for i in range(0, 5):
  new = make_add_edge_left_outter(research_orgs_doc, [researchers_doc])
  research_org_edges_doc = research_org_edges_doc + new

write_json("output_small/edges/researchers_edges.json", research_org_edges_doc)
